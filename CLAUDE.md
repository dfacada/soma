...
- Fitbit is the **Google Health API** now (the Fitbit Web API died in September 2026): never build against api.fitbit.com. Steps only so far; the server makes the Google calls because they need the client secret, and never stores a token.
