...
**In code:** tokens are CSS variables in `src/app/globals.css` (never hard-code a hex in a component); components live in `src/components/ui/` (one CSS Module, `ui.module.css`; domain colour comes from the `journal`/`food`/`activity` class setting `--hue`). `/kit` shows every token and component live: add new components there. No Tailwind, no UI library. Type roles are the global classes `d` (display), `m` (mono numbers), `eb` (eyebrow label).
