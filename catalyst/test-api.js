...
(async () => {
  ...
  check(configured ? 'auth-url asks for offline, read-only activity' : 'auth-url is 503 until the keys are set',
    configured ? r.status === 200 && r.json.url.includes('access_type=offline') && r.json.url.includes('googlehealth.activity_and_fitness.readonly') && r.json.url.includes('state=' + state) && !r.json.url.includes('secret') : r.status === 503, r.json);
