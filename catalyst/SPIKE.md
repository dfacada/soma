...
- **From David's first run of the spike page (desktop Chrome):** `catalyst.auth.signUp()` returned 200 and created the app user; a repeat returns 409 `Duplicate value for Email ID`. After email confirmation the embedded sign-in produced a session on the Slate origin (`isUserAuthenticated()` resolved with `content.user_id`). Two SDK traps: (1) with no session, `isUserAuthenticated()` rejects with the message `Some network issue occured!` on what is a plain 401, so treat any rejection as signed out and never show that text; (2) with no session, **`generateAuthToken()` never settles** (no resolve, no reject, observed 13 s+), so always gate it on `isUserAuthenticated()` and race it against a timeout. The `/me` and upload buttons were not pressed in that run, so the token format and the browser upload are still unproven.
- The one-year `max-age` already bit once: a browser that loaded `spike.html` before the Cache toggle was disabled kept serving the old copy after redeploys. Anyone who opened a page before the toggle needs a hard refresh or a `?v=N` query once.

- **Item 2 PASSES (desktop Chrome, 2026-09-16).** Signed in on the Slate origin, `generateAuthToken()` returned a 93-char token and `GET /me` with **`Authorization: <raw token>`** returned 200 with the caller's id, email, name, role and status (~700 ms cold, ~260 ms warm). `Authorization: Bearer <token>` → 401 `INVALID_TOKEN / invalid oauth token` from the gateway. Cookies only (`credentials: 'include'`) → 401 from our `withUser`: the session cookie lives on the Slate origin and never reaches `catalystserverless.com`. So the raw token header is the only way in, on every request.
- **Item 3 PASSES for upload; download proven piecewise.** Browser PUT of 25 MB straight to Stratus through a `/sign` URL: 200 in 19.4 s on David's home uplink (~10 Mbit/s; the server-side test did it in 5.6 s). The object is in `soma-drafts` at `<user_id>/spike-25mb.bin`, exactly 26,214,400 bytes, so `/sign`'s own-prefix rule held. David's log was copied before the GET half finished; a signed GET from the Slate origin returns 200 with `Access-Control-Allow-Origin` (checked with curl), so nothing is expected to fail there. The iPhone run should show the full `PASS: bytes identical` line. Plan for ~20 s per 25 MB on a home uplink: uploads need progress UI and must survive backgrounding (chunked drafts, as Glimpse did).
...
2. Desktop Chrome run done (items 2 and 3 pass). Still worth one run on the **iPhone in Safari**: sign in, **GET /me**, **Upload, download, compare**, wait for the `PASS`/`FAIL` line, **Copy log**. That is the browser everyone will use.
