...
type FormProps = { current: number; target: number; rest?: boolean; dayNumber?: number | null; onSave: (n: number) => void };

function PushForm({ current, target, rest = false, dayNumber, onSave }: FormProps) {
  ...
  return (
    <>
      <p className="muted" style={{ fontSize: 13 }}>
        {rest
          ? `${dayNumber ? `Day ${dayNumber} is` : "Today is"} a rest day. Anything you do still counts as activity.`
      ...
      <form className={a.pushRow} onSubmit={(e) => { e.preventDefault(); if (!Number.isNaN(n) && n >= 0) onSave(Math.min(n, 100000)); }}>
        ...
        <Input className={a.pushInput} inputMode="numeric" pattern="[0-9]*" value={value} placeholder={rest ? "0" : String(target)} aria-label="Push-ups today" onChange={(e) => setValue(e.target.value.replace(/\D/g, "").slice(0, 6))} />
